...
use esp_wifi::esp_now::{EspNow, BROADCAST_ADDRESS, ReceiveInfo};
...
fn handle_esp_now_receive(receive_info: &ReceiveInfo, data: &[u8]) {
    println!("Received ESP-NOW message from: {:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}", 
             receive_info.src_address[0], receive_info.src_address[1], 
             receive_info.src_address[2], receive_info.src_address[3],
             receive_info.src_address[4], receive_info.src_address[5]);
...
fn loop_fn() {
    unsafe {
        ...
        if let Some(ref mut uart) = UART2 {
            ...
            if i2_val > 0 && i2_val % 32 == 0 { // Обрабатываем каждые 32 байта
                process_uart_buffer();
                *addr_of_mut!(I2) = 0;
            }
        }
